...
function DynamodbAdapter(logger, opts) {
  ...
  if ('endpoint' in opts) docClientOptions.endpoint = opts.endpoint;
...
proto.get = function (key, clientKey) {
  ...
  return new RSVP.Promise(function(resolve, reject) {
    self.docClient.get({
      ...
      if (err) {
        console.error("atlassian-connect-express-dynamodb get failed", err);
        reject(err);
      } else {
        if ('val' in data) {
          resolve(data.val);
...
proto.set = function (key, value, clientKey) {
  ...
  return new RSVP.Promise(function(resolve, reject) {
    return self.docClient.put({
      ...
      if (err) {
        console.error("atlassian-connect-express-dynamodb set failed", err);
        reject(err);
      } else {
        resolve(value);
      }
...
proto.del = function (key, clientKey) {
  ...
  var createErrorHandler = function(reject) {
    return function(err) {
      console.error("atlassian-connect-express-dynamodb del failed", err);
      reject(err);
    }
  };
  ...
  if (arguments.length < 2) {
    ...
    return new RSVP.Promise(function (resolve, reject) {
      self._query_by_client_key(clientKey).catch(createErrorHandler(reject)).then(function (data) {
        var keys = [];
        for (var i = 0; i < data.Items.length; i++) {
          var item = data.Items[i];
          keys.push(item.key);
        }
        self._del_multiple(keys, clientKey).then(resolve).catch(createErrorHandler(reject));
      });
    });
  } else {
    return new RSVP.Promise(function (resolve, reject) {
      self._del_multiple([key], clientKey).then(resolve).catch(createErrorHandler(reject));
    });
  }
};
